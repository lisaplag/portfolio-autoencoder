...
def advanced_autoencoder(x_in,x, epochs, batch_size, activations, depth, neurons):
    ...
    for n in range(depth, 1, -1):
        autoencoder.add(Dense(int(neurons/np.power(2,(n-1)))))
        autoencoder.add(function)
    # output layer
...
in_fraction=int(0.7*num_obs)
x_in=dataset.iloc[:in_fraction,:50]
...
labda=0.94
s=500
x=np.matrix(dataset.iloc[:,:50])
x=np.zeros((num_obs,51))
x[:,:50]=dataset.iloc[:,:50]
x[:,50]=y.iloc[:,1]
